...
function display_systemstatus($table, $table_friendly_name) {
    ...
    $query = "SELECT * FROM " . $table . " ORDER BY `System ID` DESC LIMIT 0, 50";
...
function display_readingstable($table, $table_friendly_name) {
    ...
    //Get everything except the first row.
    $query = "SELECT * FROM " . $table . " ORDER BY ID DESC, `Tick` DESC, `Probe ID` DESC, `Measure Time` DESC LIMIT 0, 50";
